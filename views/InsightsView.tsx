  ...
  return (
    <div className="max-w-6xl mx-auto space-y-12 animate-in fade-in duration-700 pb-20">
      <div className="flex items-end justify-between">
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 px-6 py-2.5 bg-[#6C5DD3] hover:bg-[#6C5DD3] text-white rounded-xl text-sm font-black transition-all shadow-xl shadow-orange-500/20"
          >
      ...
      <div className="grid md:grid-cols-4 gap-6">
        {[
          ...
          <div key={metric.label} className={`p-8 rounded-[2rem] border transition-all shadow-lg group ${isDarkMode
            ...
            <div className={`text-[10px] font-black uppercase tracking-widest mb-3 transition-colors ${isDarkMode ? 'text-zinc-500 group-hover:text-[#6C5DD3]' : 'text-gray-500 group-hover:text-[#6C5DD3]'
              }`}>{metric.label}</div>
            ...
            <div className={`text-xs font-bold ${metric.color === 'green' ? 'text-green-500' :
                metric.color === 'red' ? 'text-red-500' : 'text-[#6C5DD3]'
              }`}>
              {metric.delta}
            </div>
          </div>
      ...
      <section className={`p-10 rounded-[3rem] border relative overflow-hidden group ${isDarkMode
          ? 'bg-gradient-to-br from-[#6C5DD3]/10 via-red-600/5 to-transparent border-[#6C5DD3]/20'
          : 'bg-gradient-to-br from-orange-50 via-red-50 to-white border-orange-200'
        }`}>
        <div className="absolute top-0 right-0 p-10 opacity-5 group-hover:scale-110 transition-transform duration-1000">
          <Radar className="w-48 h-48 text-[#6C5DD3]" />
        </div>
        <div className="flex items-center gap-4 relative z-10 mb-8">
          <div className="w-14 h-14 rounded-2xl bg-[#6C5DD3] flex items-center justify-center shadow-2xl">
            <Activity className="w-7 h-7 text-white" />
          </div>
          <div>
            <h2 className={`text-3xl font-black ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Sales Management Insight</h2>
            <p className="text-[#6C5DD3] font-bold uppercase text-[10px] tracking-widest mt-1">Autonomous Strategy Review</p>
          </div>
        </div>
